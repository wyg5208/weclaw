...
用于管理 WeClaw 离线时 PWA 用户发送的消息，支持服务器重启后数据不丢失。
"""
...
class OfflineMessageQueue:
    ...
    async def flush_to_weclaw(self, user_id: str, weclaw_session_id: str, bridge_manager) -> int:
        """WeClaw 上线后发送所有待处理消息
        
        Args:
            user_id: 用户 ID
            weclaw_session_id: WeClaw 会话 ID
        ...
        try:
            ...
            for row in rows:
                try:
                    ...
                    # 发送到 WeClaw
                    await bridge_manager.send_to_weclaw(weclaw_session_id, {
                        "type": "chat",
