...
连接远程服务与 WeClaw 本地实例，提供消息转发和事件同步。
"""
...
class WeClawBridge:
    ...
    负责将远程客户端的请求转发到本地 WeClaw 实例，
    并将 WeClaw 的事件推送到远程客户端。
    """
    ...
    def __init__(
        ...
        Args:
            agent: WeClaw Agent 实例
            event_bus: WeClaw EventBus 实例
            session_manager: WeClaw SessionManager 实例
        ...
        logger.info("WeClaw 桥接器初始化完成")
    
    def _setup_event_forwarding(self):
        """设置事件转发 - 将 WeClaw 事件转发到远程客户端"""
    ...
    async def process_message(
        ...
        if not self.agent:
            yield {
                "type": "error",
                "payload": {"code": "NOT_CONNECTED", "message": "WeClaw 未连接"}
            }
            return
    ...
    async def _download_attachment_to_temp(self, url: str, attachments: list) -> str:
        ...
        try:
            ...
            temp_dir = Path(tempfile.gettempdir()) / "weclaw_pwa_attachments"
