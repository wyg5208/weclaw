...
from .bridge.weclaw_bridge import WeClawBridge
...
async def lifespan(app: FastAPI):
    ...
    # 初始化 WeClaw 桥接器（延迟初始化，等待 WeClaw 实例）
    # weclaw_bridge 将在 WeClaw 启动后通过 set_weclaw_instance 设置
    logger.info("等待 WeClaw 实例连接...")
...
# 创建 FastAPI 应用
app = FastAPI(
    title="WeClaw Remote Server",
    description="WeClaw 远程访问 API 服务",
    version="1.0.0",
    lifespan=lifespan
)
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "version": "1.0.0",
        "weclaw_connected": has_connection
    }
...
# Bridge WebSocket 端点 - WeClaw 桌面端连接
@app.websocket("/ws/bridge")
async def bridge_endpoint(websocket: WebSocket):
    """WeClaw 桌面端 Bridge 连接端点
...
def set_weclaw_instance(agent, event_bus, session_manager):
    """设置 WeClaw 实例（由 WeClaw 主程序调用）"""
    conn_mgr = context.get_connection_manager()
    ...
    context.set_weclaw_bridge(bridge)
    logger.info("WeClaw 实例已连接到远程服务")


def get_weclaw_bridge() -> Optional[WeClawBridge]:
    """获取 WeClaw 桥接器实例"""
    return context.get_weclaw_bridge()
