...
class BindingTokenResponse(BaseModel):
    ...
    message: str = "请在 WeClaw PC 端输入此 Token 完成绑定"
...
async def generate_binding_token(user_info: dict = Depends(get_current_user_with_db)):
    ...
    用户在 PWA 端生成 Token，然后在 WeClaw PC 端输入 Token 完成绑定。
    每个用户只能绑定一个 WeClaw PC。
    """
    user = user_info["user"]
    user_manager = context.get_user_manager()
...
async def bind_device(request: BindDeviceRequest):
    ...
    WeClaw PC 端调用此接口，使用用户提供的绑定 Token 完成绑定。
    ...
    success = user_manager.bind_device(
        ...
        device_id=device_id,
        device_name=request.device_name or f"WeClaw-{device_id[:8]}",
        device_fingerprint=request.device_fingerprint
    )
