...
async def send_message(
    ...
    # 更新请求计数和活跃用户记录
    from .. import context
    from ..websocket.bridge_handler import get_bridge_manager, send_pwa_status_to_all_weclaws
    ...
    # [建议B] 消息存入 pending 即可返回，真正的 WeClaw 发送在 /stream SSE 端点中进行
    # 不在此处提前发送，避免消息被发送两次导致 WeClaw 处理两次的 Bug
...
async def stream_response(
    ...
    async def generate() -> AsyncGenerator[str, None]:
        ...
        try:
            # 方式1: 本地 WeClaw Bridge（内嵌模式）
            bridge = context.get_weclaw_bridge()
            ...
            # 方式2: 远程 WeClaw Bridge（独立服务器模式）
            else:
                from ..websocket.bridge_handler import get_bridge_manager
                bridge_manager = get_bridge_manager()
                # 使用用户的 user_id 获取其绑定的 WeClaw 连接
                connection = bridge_manager.get_primary_connection(user.user_id)
                
                if not connection:
                    yield f"event: error\ndata: {{\"code\": \"NO_WECLAW\", \"message\": \"您的 WeClaw PC 未连接或未绑定\"}}\n\n"
                    return
                
                # 发送思考状态
                yield f"event: thinking\ndata: {{\"content\": \"正在发送到 WeClaw...\"}}\n\n"
                ...
                try:
                    ...
                    await bridge_manager.send_to_weclaw(connection.session_id, request_msg)
                    logger.info(f"消息已转发到 WeClaw: message_id={message_id[:8]}, request={request_id[:8]}")
                    ...
                    while True:
                        try:
                            ...
                            yield f"event: error\ndata: {{\"code\": \"TIMEOUT\", \"message\": \"WeClaw 响应超时\"}}\n\n"
                            break
