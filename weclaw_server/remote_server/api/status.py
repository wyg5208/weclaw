...
@router.get("/status", summary="获取 WeClaw 状态")
...
async def get_tool_detail(tool_name: str):
    ...
    if not bridge or not bridge.is_connected():
        raise HTTPException(status_code=503, detail="WeClaw 未连接")
...
async def health_check():
    ...
    return {
        ...
        "components": {
            "server": "ok",
            "weclaw": "connected" if bridge and bridge.is_connected() else "disconnected",
