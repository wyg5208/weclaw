...
async def websocket_endpoint(
    ...
    weclaw_bridge = context.get_weclaw_bridge()
    ...
    # 通知 WeClaw 有新的 PWA 连接
    from .bridge_handler import send_pwa_status_to_all_weclaws
    await send_pwa_status_to_all_weclaws()
...
async def handle_chat_message(
    ...
    attachments: list,
    connection_manager,
    weclaw_bridge,  # 已废弃，使用 bridge_handler 代替
    request_id: str = None  # ✅ 新增：客户端传来的 request_id
):
    """处理聊天消息 - 转发到用户绑定的 WeClaw PC 端"""
    ...
    # 查找用户绑定的 WeClaw 连接（包含设备指纹匹配）
    weclaw_conn = bridge_manager.get_user_connection(user_id)
    ...
    if not weclaw_conn:
        ...
        if has_bound_device:
            ...
            message = t("device_offline_msg", lang, default="💻 您的 WeClaw PC 端当前离线，消息将在其上线后自动发送")
            
            # ✅ 新增：同时通过 WebSocket 推送状态通知
            from .bridge_handler import broadcast_weclaw_status_change
            asyncio.create_task(
                broadcast_weclaw_status_change(user_id, "offline")
            )
        else:
            # 未绑定设备
            error_code = "NO_DEVICE_BOUND"
            lang = get_user_language(user_id)
            message = t("no_device_bound_msg", lang, default="📱 您还没有绑定 WeClaw PC 端，请在设置中完成设备绑定")
    ...
    try:
        ...
        # 发送消息到 WeClaw PC 端
        await bridge_manager.send_to_weclaw(weclaw_conn.session_id, {
            "type": "chat",
        ...
        # logger.info(f"消息已转发到 WeClaw: user={user_id[:8]}, request={request_id[:8]}")  # 调试用
...
async def handle_stop_generation(
    ...
    """处理停止生成请求"""
    if weclaw_bridge:
        stopped = weclaw_bridge.stop_generation(user_id)
