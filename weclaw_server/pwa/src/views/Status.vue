<template>
  <div class="status-page">
    ...
    <!-- WeClaw 状态 -->
    <van-cell-group title="WeClaw 状态" inset>
      <van-cell title="运行状态" :value="weclawStatus.status">
        <template #icon>
          <van-icon :name="getStatusIcon(weclawStatus.status)" :color="getStatusColor(weclawStatus.status)" size="20" style="margin-right: 8px" />
        </template>
      </van-cell>
      <van-cell title="版本" :value="weclawStatus.version" />
      <van-cell title="运行时间" :value="formatUptime(weclawStatus.uptime_seconds)" />
      <van-cell title="当前模型" :value="weclawStatus.model?.name || '无'" />
    </van-cell-group>

    <!-- 当前任务 -->
    <van-cell-group v-if="weclawStatus.current_task" title="当前任务" inset>
      <van-cell title="任务类型" :value="weclawStatus.current_task.type" />
      <van-cell title="任务状态" :value="weclawStatus.current_task.status" />
      <van-cell title="开始时间" :value="formatTime(weclawStatus.current_task.started_at)" />
      <van-cell v-if="weclawStatus.current_task.progress" title="进度">
        <van-progress :percentage="weclawStatus.current_task.progress" />
      </van-cell>
    </van-cell-group>
