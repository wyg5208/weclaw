...
export const useDeviceStore = defineStore('device', () => {
  ...
  // ✅ 新增：WeClaw 实时状态（Phase 1.3）
  const weclawStatus = ref<'online' | 'offline' | 'reconnecting' | 'unknown'>('unknown')
  ...
  const comprehensiveOnlineStatus = computed(() => {
    // 优先使用实时 WebSocket 状态
    if (weclawStatus.value !== 'unknown') {
      return weclawStatus.value === 'online'
    }
    // 降级使用最后连接时间判断
  ...
  // ✅ 新增：更新 WeClaw 实时状态（Phase 1.3）
  function updateWeClawStatus(status: 'online' | 'offline' | 'reconnecting' | 'unknown') {
    weclawStatus.value = status
