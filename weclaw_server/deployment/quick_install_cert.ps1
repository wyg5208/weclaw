# WeClaw Let's Encrypt 证书快速安装指南
# 按照步骤执行即可完成证书申请和部署

Write-Host "========================================" -ForegroundColor Cyan
Write-Host " WeClaw Let's Encrypt 证书快速安装" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# ========== 第 1 步：创建目录 ==========
Write-Host "[第 1 步] 创建必要目录..." -ForegroundColor Yellow
New-Item -ItemType Directory -Path "D:\www\weclaw" -Force | Out-Null
New-Item -ItemType Directory -Path "D:\www\weclaw\.well-known\acme-challenge" -Force | Out-Null
...
.\wacs.exe --target manual --host "weclaw.cc,*.weclaw.cc" --webroot "D:\www\weclaw" --store pemfiles --pemfilespath "D:\nginx\ssl\weclaw.cc" --accepttos --verbose
...
Write-Host ""
Write-Host "1. 更新 Nginx 配置启用 HTTPS（编辑 deployment/nginx/weclaw.conf）" -ForegroundColor White
