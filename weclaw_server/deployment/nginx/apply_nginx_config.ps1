...
Write-Host "WeClaw Nginx 配置修复工具" -ForegroundColor Cyan
Write-Host "======================================" -ForegroundColor Cyan
Write-Host ""
...
if ($LASTEXITCODE -eq 0) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "3. ✓ 已移除 weclaw_http.conf 中的 default_server 标记" -ForegroundColor White
        Write-Host "4. ✓ 已正确导入 weclaw.conf（包含完整 HTTPS 配置）" -ForegroundColor White
        Write-Host ""
