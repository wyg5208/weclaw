...
interface WebSocketOptions {
  ...
  onConnect?: () => void
  onDisconnect?: () => void
  onWinClawStatusChange?: (status: 'online' | 'offline' | 'reconnecting' | 'unknown') => void  // ✅ 新增（Phase 1.3）
  reconnect?: boolean
...
export function useWebSocket(options: WebSocketOptions = {}) {
  const {
    ...
    onConnect,
    onDisconnect,
    onWinClawStatusChange,  // ✅ 新增（Phase 1.3）
    reconnect = true,
      ...
      try {
        ...
        switch (data.type) {
          ...
          case 'winclaw_status':
            ...
            if (payload?.status) {
              onWinClawStatusChange?.(payload.status as 'online' | 'offline' | 'reconnecting' | 'unknown')
            }
            break
          default:
