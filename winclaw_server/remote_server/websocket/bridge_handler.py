...
处理来自 WinClaw 桌面端的 WebSocket 连接，实现：
1. WinClaw 实例注册和管理
2. 消息路由（将 PWA 用户请求转发到对应 WinClaw）
3. 状态广播
"""
...
class BridgeConnectionManager:
    ...
    def __init__(self):
        # session_id -> WinClawConnection
        self._connections: dict[str, WinClawConnection] = {}
    ...
    async def connect(
        ...
        logger.info(f"WinClaw 已连接：session={session_id[:8]}, device={device_id[:8] if device_id else 'unknown'}, user={user_id[:8] if user_id else 'unbound'}, fingerprint_valid={bool(user_id)}")
    ...
    async def _flush_offline_messages(self, user_id: str, session_id: str):
        ...
        try:
            ...
            sent_count = await queue.flush_to_winclaw(user_id, session_id, self)
                
            if sent_count > 0:
                logger.info(f"已发送 {sent_count} 条离线消息到 WinClaw: user={user_id[:8]}")
    ...
    def disconnect(self, session_id: str):
        ...
        if connection:
            ...
            logger.info(f"WinClaw 已断开：session={session_id[:8]}")
    ...
    def get_connection(self, session_id: str) -> Optional[WinClawConnection]:
        """获取连接"""
        return self._connections.get(session_id)
    
    def get_connection_by_device(self, device_id: str) -> Optional[WinClawConnection]:
        """根据设备 ID 获取连接"""
    ...
    def get_primary_connection(self, user_id: str) -> Optional[WinClawConnection]:
        """获取用户绑定的主 WinClaw 连接（第一个连接的）"""
    ...
    async def send_to_winclaw(
        ...
        try:
            ...
            logger.error(f"发送到 WinClaw 失败: {e}")
            return False
    
    async def send_to_user_winclaws(
        self,
        user_id: str,
        message: dict
    ) -> list[str]:
        """发送消息到用户的所有 WinClaw 实例"""
        sent_to = []
        
        for conn in self.get_user_connections(user_id):
            if await self.send_to_winclaw(conn.session_id, message):
                sent_to.append(conn.session_id)
        
        return sent_to
    
    async def request_to_winclaw(
        self,
        session_id: str,
        message: dict,
        timeout: float = 300.0
...
async def bridge_websocket_endpoint(
    ...
    try:
        ...
        await websocket.send_json({
            ...
            "message": "WinClaw 桥接连接成功" if connection.user_id else "WinClaw 已连接，设备未绑定或指纹验证失败"
        })
        
        # 发送当前 PWA 连接状态
        await send_pwa_status_to_winclaw(websocket)
        ...
        logger.info(f"WinClaw 断开连接: session={session_id[:8]}")
    except Exception as e:
...
async def handle_bridge_message(connection: WinClawConnection, message: dict):
    """处理来自 WinClaw 的消息"""
    ...
    if msg_type == "pong":
        ...
        # 日志消息
        logger.info(f"[WinClaw] {payload}")
    
    elif msg_type == "reconnected":
        # WinClaw 重连通知
        device_id = payload.get("device_id", "unknown")
        logger.info(f"WinClaw 重连成功: session={connection.session_id[:8]}, device={device_id}")
...
async def send_pwa_status_to_winclaw(websocket: WebSocket):
    """发送 PWA 连接状态到 WinClaw"""
    from .. import context
...
async def send_pwa_status_to_all_winclaws():
    """发送 PWA 连接状态到所有 WinClaw 连接"""
    from .. import context
    ...
    for session_id, conn in list(bridge_manager._connections.items()):
        try:
            ...
            logger.warning(f"发送 PWA 状态到 WinClaw 失败：{e}")
