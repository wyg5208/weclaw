...
- **状态监控**：远程查看 WinClaw 运行状态、当前任务、可用工具
...
**核心功能：FastAPI 远程服务端，支持移动端远程访问 WinClaw**
...
5. **remote_server/bridge/** - WinClaw 桥接
...
**PWA-WinClaw 双向通信闭环修复 + 多设备消息隔离**

1. **消息响应闭环修复**：
   - 统一 request_id 路由机制，使用 message_id 作为 request_id
   - 修复 WinClaw 响应格式：payload.content 替代 delta，stream_end 替代 done
...
2. **服务器端 Bridge 端点**：
   - `/ws/bridge` WebSocket 端点接收 WinClaw 连接
   - BridgeConnectionManager 管理多用户多实例
   - 消息路由（PWA 请求 → 对应用户的 WinClaw）
   - 双向通信支持

3. **架构更新**：
   - 支持独立服务器模式（服务器与 WinClaw 分离）
   - 支持内嵌模式（服务器与 WinClaw 同一进程）
   - Chat API 自动检测连接模式
