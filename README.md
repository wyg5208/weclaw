...
#### 语音连续播放修复（pyttsx3 全局缓存清理）🔊
- ✅ 修复语音输出工具连续调用只有第一次有声音的问题
- ✅ 定位根因：pyttsx3 内部 `_activeEngines` 全局缓存导致后续实例损坏
- ✅ 在引擎创建前后手动清理 `_activeEngines` 缓存
- ✅ 显式指定 `driverName='sapi5'` 强制创建全新引擎实例
- ✅ qasync 线程池环境中显式 COM 初始化/反初始化
- ✅ `save_to_file` 和 `list_voices` 接口同步修复

### 技术改进
- 📝 `_do_speak` 新增线程 ID 诊断日志
- 🎨 finally 块双重保障：`engine.stop()` + `_activeEngines.clear()` + `del engine`
...
**一句话总结**：Weclaw = **本地版 ChatGPT + 22 个专业工具 + 手机远程控制**，同等能力，更低成本，更高隐私！
