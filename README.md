...
#### PPT工具真实图片插入功能增强 🖼️
- ✅ PPT工具支持真实图片插入 - 新增 `_fill_image_slide` 方法
- ✅ 支持多种图片布局：left_half、right_half、left_large、right_large、full_bg、top_bottom
- ✅ grid_images 类型支持 images 参数传入多张图片（1/2/3/4/6张网格布局）
- ✅ 修复参数名兼容问题（image_layout/layout）
- ✅ 图片不存在时自动回退到占位符图标
- ✅ 全屏背景图自动移到最底层并叠加半透明遮罩

### 技术改进 📝
- 📝 新增 `add_image_placeholder` 函数，支持真实图片插入和占位符回退
- 📝 参数兼容处理：`config.get("image_layout", config.get("layout", "left_half"))`
- 📝 全屏背景图处理：操作底层 XML 元素将图片移到最底层
- 📝 涉及文件：`src/tools/ppt_generator.py`
...
- ✅ Excel 公式验证模块 - 支持重算验证、引用检查、专业样式（4种主题）
- ✅ DOCX 验证模块 - 6 个检测器（表格一致性、图片纵横比、批注链接、书签完整性、绘图ID、超链接）
- ✅ PDF 生成工具 - HTML/Markdown 转 PDF，支持多种页面样式
- ✅ DOCX 模板系统 - 5 种专业模板（学术论文、商业报告、会议纪要、合同、简历）
- ✅ PPT 图表幻灯片 - 支持柱状图、折线图、饼图等，章节分隔页
- ✅ 综合测试体系 - 47 项测试 100% 通过

#### Bug 修复 🐛

- 修复 openpyxl 版本兼容性问题（FORMAT_NUMBER_COMMA_SEP1 已移除）
- 修复模板字典访问错误（colors.primary → colors["primary"]）
...
#### 复制对话按钮视觉反馈优化 📋
- ✅ 点击"复制对话"按钮后，按钮文字变为"✓ 已复制"
- ✅ 按钮临时禁用防止重复点击
- ✅ 底部状态栏显示"已复制到剪贴板"（2秒后自动消失）
- ✅ 1.5秒后按钮状态自动恢复

#### 工具状态面板刷新逻辑优化 🔧
- ✅ 移除每次AI回复时的日志清空逻辑
- ✅ 改为只在新建会话时清空工具日志
- ✅ 现在可以追踪完整对话过程中的所有工具执行记录
- ✅ 新建会话后工具状态面板正确重置

#### 工具 output_dir 配置修复 📁
- ✅ 修复7个工具的 output_dir 配置覆盖日期文件夹问题
- ✅ 受影响工具：contract_generator、financial_report、resume_builder、mind_map、speech_to_text、education_tool、coding_assistant
- ✅ 工具生成文件现在正确保存在 `generated/YYYY-MM-DD/` 目录

### 技术改进 📝
- 📝 涉及文件：`src/ui/main_window.py`、`src/ui/gui_app.py`、`config/tools.json`
...
#### 课程表工具智能姓名纠错与昵称映射 🧠
- ✅ 新增拼音相似度算法：支持同音字自动纠正（温/文、佳/家、露/路等）
- ✅ 新增字符串编辑距离相似度算法：综合评分匹配
- ✅ 新增昵称映射表 NICKNAME_MAP：支持小名→正式名映射（小鹿儿→翁迦鹿、溪溪→小溪溪等）
- ✅ 实现 `_find_best_matching_member` 智能匹配方法：三级匹配策略（昵称映射 > 精确匹配 > 模糊匹配）
- ✅ 所有课程表操作支持姓名纠错：search_courses、add_course、edit_course、delete_course
- ✅ 用户友好反馈：匹配失败时提供可用姓名列表和昵称示例，匹配成功时提示姓名纠正情况
- ✅ 匹配算法：拼音相似度 70% + 字符串相似度 30%，阈值 0.7

### 技术改进 📝
- 📝 新增 `NICKNAME_MAP` 常量字典，支持动态扩展昵称映射
- 📝 新增 `_calculate_pinyin_similarity()` 函数（pypinyin + SequenceMatcher）
- 📝 新增 `_calculate_string_similarity()` 函数（编辑距离算法）
...
### v2.29.0 (2026-03-28)
- ✅ PPT工具支持真实图片插入 - 新增 `_fill_image_slide` 方法
- ✅ 支持 6 种图片布局：left_half、right_half、left_large、right_large、full_bg、top_bottom
- ✅ grid_images 支持多图网格（1/2/3/4/6张），新增 images 参数
- ✅ 修复参数名兼容问题（image_layout/layout）
- ✅ 图片不存在时自动回退到占位符，全屏背景图处理优化
...
---
### v2.24.0 (2026-03-27)
- ✅ 课程表工具智能姓名纠错与昵称映射功能开发
- ✅ 新增拼音相似度算法和字符串编辑距离算法
- ✅ 新增昵称映射表，支持小名→正式名自动映射
- ✅ 实现三级匹配策略（昵称映射 > 精确匹配 > 模糊匹配）
- ✅ 所有课程表操作支持姓名纠错和模糊匹配
- ✅ 匹配算法：拼音相似度 70% + 字符串相似度 30%，阈值 0.7
- ✅ 用户友好提示：匹配失败时提供可用姓名和昵称示例
...
#### 家庭大事记工具 🎊
- ✅ 记录家庭重大事件、纪念日、议程
- ✅ 7种事件类型：纪念日、庆典、里程碑、议程、成就、回忆、其他
- ✅ 6种展示模板：时间线、卡片、日历、相册、列表、年度报告
- ✅ 年度纪念日自动重复提醒
- ✅ 关联家庭成员、地点、照片
...
#### 歌曲智能匹配算法 🧠
- ✅ 新增 `_find_best_match()` 方法，使用评分制选择最佳匹配
- ✅ 文本标准化：统一括号、去空格、转小写
- ✅ 匹配策略：完全匹配 (100 分)、包含匹配 (50 分)、关键词匹配 (10 分/个)
- ✅ 艺术家匹配额外加分 (50 分)
- ✅ 解决用户说"播放 XXX"却播放错误歌曲的问题

#### 迷你播放器 UI 优化 🎨
- ✅ 规范为两种固定尺寸模式：迷你 (320×100)、展开 (450×400)
- ✅ 使用 `setFixedSize()` 替代可伸缩布局，避免拖拽变形
- ✅ 优化模式切换逻辑，增加日志记录和 Tooltip 提示
- ✅ 修复底部进度条重复显示问题

### Bug修复 🐛

#### 设置对话框崩溃修复 🔧
- ✅ 修复歌曲库分组布局错误（`QVBoxLayout` → `QFormLayout`）
- ✅ 解决打开设置对话框时报 `AttributeError` 的问题
- ✅ 用户可以正常配置默认音乐文件夹
...
#### 主窗口关闭后迷你播放器后台残留问题修复 🔧
- ✅ 修复主窗口关闭后迷你播放器窗口仍在后台运行的问题
- ✅ 新增 `_close_music_player()` 方法，确保播放器正确停止播放
- ✅ 调用 `deleteLater()` 释放 Qt 窗口资源

### 功能新增 🎉

#### 迷你音乐播放器 🎵
- ✅ 新增完整音乐播放器工具：歌曲库管理、播放控制、标签系统
- ✅ 迷你播放器集成到主界面工具栏（歌曲库按钮）
- ✅ 支持播放控制（播放/暂停、切歌、循环、随机）
- ✅ 歌曲库管理（添加、删除、搜索、扫描本地音乐）
- ✅ 标签系统：内置标签 + 自定义标签分类
- ✅ Audio Ducking 与 TTS 协调播放
...
#### 新建会话AI消息残留问题修复 🔧
- ✅ 修复 ChatWidget.clear() 循环条件错误（`> 1` → `> 0`），真正清空所有消息气泡
...
#### 语音对话TTS冲突修复 🎙️
- ✅ 录音按钮点击时立即停止正在播放的TTS，避免新旧对话语音混淆
- ✅ 修复流式TTS播放中途恢复监听问题，确保队列全部播完后才恢复监听
- ✅ Agent调用录音工具入口同样停止TTS播放

### 功能优化 ✨

#### 英语口语对话精简 📚
- ✅ 优化系统prompt，强制AI回复简洁（1-2句）
- ✅ 移除冗余的选项列表和重复说明
- ✅ 提升对话练习的流畅性和自然度
...
### v2.21.3 (2026-03-24)
- ✅ 修复主窗口关闭后迷你播放器后台残留问题
- ✅ 新增 `_close_music_player()` 方法确保播放器正确停止
- ✅ 调用 `deleteLater()` 释放 Qt 窗口资源
- ✅ 迷你音乐播放器开发：歌曲库管理、播放控制、标签系统
- ✅ 迷你播放器集成到主界面工具栏
- ✅ Audio Ducking 与 TTS 协调播放
...
- ✅ 新建会话时取消正在进行的聊天任务，防止信号残留
- ✅ Agent.reset() 改为创建新会话而非仅清空消息
- ✅ 旧会话自动保存到历史记录，新建会话后自动刷新历史 TAB
