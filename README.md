...
#### 桌面端 UI 布局全面优化 🎨
- ✅ 窗体默认位置上移 50 像素，确保底栏完整显示
- ✅ 底栏 Token 显示拆分为输入/输出，PWA 连接状态移到底栏
- ✅ 右侧面板简化：删除 Token 用量和 PWA 连接卡片，工具执行状态扩展
- ✅ 消息气泡图标改为右上角横向排列，减少最小行高
- ✅ 输入栏默认一行，支持展开/收起，新增复制/粘贴/清空按钮
- ✅ 消息列表向上紧凑对齐，避免垂直均匀分布
- ✅ 右侧面板初始宽度翻倍（300px），左侧消息区相应变窄

### 技术改进
- 📝 按钮使用纯文字替代 Emoji 确保跨平台兼容
- 🎨 按钮样式使用透明背景 + 黑色边框，适配所有主题
